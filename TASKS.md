...
- [x] `P27-001` Add optional `worktreePath: string` field to `PhaseSchema` in `src/types/index.ts`. When set, the phase runner uses this path as its working directory instead of `projectRootDir`. Null/absent means the phase uses the project root.
...
- [x] `P27-007` Use `activePhaseIds: string[]` in `ProjectStateSchema`. Update `resolveActivePhaseStrict` to resolve a target phase by ID from the set. Update `ixado phase active` CLI to add/remove IDs from the set. Deps: `P27-006`.
...
- [x] `P34-010` Use `GateChain` execution in `PhaseRunner`. Added `GateConfigSchema` discriminated union, `gates` field to `ExecutionLoopSettingsSchema`, `createGatesFromConfig` factory, and `runPostIntegrationGateChain` in PhaseRunner. All 1134 tests pass. Deps: `P34-006`..`P34-009`.
...
- [x] `P34-013` Regression tests in `src/engine/p34-013-regression.test.ts`: VcsProvider routing, gate factory, GateConfig schema validation, gate chain sequencing, gate event emission, and gate failure notification surfacing. Deps: `P34-004`, `P34-010`, `P34-011`, `P34-012`.
...
- [x] `P35-004` Wire `RaceOrchestrator` into `PhaseRunner`: when `race > 1`, fan out to N branches; after all complete, run judge; merge/cherry-pick winner; prune loser worktrees. Fall back to single execution when `race` is unset or 1. Deps: `P35-002`, `P35-003`.
