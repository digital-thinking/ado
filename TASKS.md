...
- [ ] `P15-001` Extract phase-run orchestration from `src/cli/index.ts` into a dedicated runner module (`src/engine/phase-runner.ts`) with single-responsibility steps for branch preparation, task execution, recovery handling, tester/fixer loop, and CI integration. Keep CLI command wiring thin. Deps: `P14-009`.
...
- [ ] `P15-006` Optimize `/api/agents` recovery enrichment path to avoid loading every project state on each poll; introduce cached/indexed recovery metadata updated on state writes and validate parity with current response payload. Deps: `P15-005`.
