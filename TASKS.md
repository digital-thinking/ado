...
- [ ] `P16-003` Refactor clean-worktree gate to ignore IxADO-owned runtime artifacts (`.ixado/` transient files) while still failing on real source changes. Keep a single-path dirty check contract in `GitManager`. Deps: `P15-009`. **Status: InProgress**.
