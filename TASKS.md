...
- [ ] `P15-006` Refactor `/api/agents` enrichment path to be side-effect free and scalable: remove task-state mutation from the GET handler, move failure reconciliation to explicit orchestration/update hooks, and avoid loading every project state on each poll by using cached/indexed recovery metadata updated on state writes. Validate response parity. Deps: `P15-005`.
