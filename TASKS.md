...
- [ ] `P15-003` Introduce a shared strict JSON extraction/parser utility for model outputs in `src/engine/` and reuse it across recovery and TASKS.md import paths to remove duplicated extraction logic and keep one contract-compliant parser path. **Status: InProgress**. Deps: `P15-001`.
